package com.atguigu.refreshlistview_library;
...
public class RefreshListView extends ListView {
    ...
    private int listViewOnScreenY = -1;//listView在Y轴上的坐标
    ...
    private void initFooterView(Context context) {
        ...
        footerView.setPadding(0, -footerViewHeight, 0, 0);//默认隐藏
        //ListView添加Footer
    ...
    /*
    * 判断是否完全显示顶部轮播图
    * 当ListView在屏幕上的Y轴坐标小于或等于顶部轮播图在屏幕上的Y轴坐标,完全显示顶部轮播图
    * */
    private Boolean isDisplayTopNews() {
        if (topNewsView != null) {
            //1.得到ListView在屏幕上的Y轴坐标
    ...
    private void refreshViewState() {
        switch (currentStatus) {
            ...
            case RELEASE_REFRESH://手松刷新状态
                ...
                break;
            case REFRESHING://正在更新状态
                pb_status.setVisibility(VISIBLE);
                iv_arrow.clearAnimation();
                iv_arrow.setVisibility(GONE);
                tv_status.setText("正在更新中...");
                break;
            default:
                break;
        }
    ...
    * @params sucess
    * */
    public void setOnRefreshFinish(boolean sucess) {
        if (isLoadMore) {
            //加载更多
            isLoadMore = false;
            //隐藏
            footerView.setPadding(0, -pullDownRefreshHeight, 0, 0);
        } else {
            //下拉刷新
            tv_status.setText("下拉刷新...");
            currentStatus = PULL_DOWN_REFRESH;
